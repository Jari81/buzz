...
pub struct QueuedEvent {
    ...
    /// Source recovery event, if this event was generated by recovery.
    ///
    /// Normal relay events carry `None`. This is queue-local provenance only;
    /// it does not affect normal queue lifecycle behavior.
    pub recovery_event_id: Option<EventId>,
}
...
pub struct BatchEvent {
    ...
    pub prompt_tag: String,
    pub received_at: Instant,
    /// Source recovery event propagated unchanged from [`QueuedEvent`].
    pub recovery_event_id: Option<EventId>,
}
...
impl FlushBatch {
    /// Consume this batch, separating recovery-derived events from normal work.
    ///
    /// The returned batch retains every normal event in its original order in
    /// both `events` and `cancelled_events`. Recovery event IDs are returned in
    /// deterministic batch order: first `events`, then `cancelled_events`.
    /// This only splits data; it does not mutate queue lifecycle state.
    #[allow(dead_code)]
    pub(crate) fn split_recovery_events(self) -> (Self, Vec<EventId>) {
        let FlushBatch {
            channel_id,
            events,
            cancelled_events,
        ...
        let mut recovery_event_ids = Vec::new();
        let events = events
            .into_iter()
            .filter_map(|event| match event.recovery_event_id {
                Some(recovery_event_id) => {
                    recovery_event_ids.push(recovery_event_id);
                    None
                }
                None => Some(event),
            })
            .collect();
        let cancelled_events = cancelled_events
            .into_iter()
            .filter_map(|event| match event.recovery_event_id {
                Some(recovery_event_id) => {
                    recovery_event_ids.push(recovery_event_id);
...
impl EventQueue {
    ...
    pub fn flush_next(&mut self) -> Option<FlushBatch> {
        ...
        let mut events: Vec<BatchEvent> = queue
            ...
            .map(|qe| BatchEvent {
                ...
                recovery_event_id: qe.recovery_event_id,
            })
            .collect();
    ...
    pub fn requeue(&mut self, batch: FlushBatch) -> Option<FlushBatch> {
        ...
        let event_count = batch.events.len() + batch.cancelled_events.len();
        let attempt = {
        ...
        let queue = self.queues.entry(channel_id).or_default();
        // Push cancelled carryover then current events to the front in reverse
        // order so the original merged-batch order is preserved.
        for be in batch.cancelled_events.into_iter().chain(batch.events).rev() {
            queue.push_front(QueuedEvent {
                ...
                recovery_event_id: be.recovery_event_id,
            });
        }
...
mod tests {
    ...
    #[test]
    fn test_recovery_provenance_survives_queue_routes_and_splits_deterministically() {
        let recovery_id = make_event("recovery-origin").id;
        let queued_recovery = |channel_id, content: &str, recovery_event_id| QueuedEvent {
            channel_id,
            event: make_event(content),
            received_at: Instant::now(),
            prompt_tag: "test".into(),
        ...
        let assert_recovery = |event: &BatchEvent, expected| {
            assert_eq!(event.recovery_event_id, Some(expected));
        };

        // push → flush → ordinary requeue → timestamp-preserving requeue.
        let mut queue = EventQueue::new(DedupMode::Queue);
        let channel_id = Uuid::new_v4();
        queue.push(queued_recovery(channel_id, "requeue", recovery_id));
        let batch = queue.flush_next().expect("initial recovery batch");
        assert_recovery(&batch.events[0], recovery_id);
        assert!(queue.requeue(batch).is_none());
        queue.mark_complete(channel_id);
        queue.retry_after.remove(&channel_id);
        let batch = queue.flush_next().expect("ordinary requeue recovery batch");
        assert_recovery(&batch.events[0], recovery_id);
        queue.requeue_preserve_timestamps(batch);
        queue.mark_complete(channel_id);
        let batch = queue
            .flush_next()
            .expect("timestamp-preserving recovery batch");
        assert_recovery(&batch.events[0], recovery_id);

        // Cancelled merge preserves recovery provenance in cancelled_events.
        queue.push(make_queued(channel_id, "normal merge event"));
        queue.requeue_as_cancelled(batch, CancelReason::Steer);
        queue.mark_complete(channel_id);
        let merged = queue.flush_next().expect("merged recovery batch");
        assert_eq!(merged.events[0].recovery_event_id, None);
        assert_recovery(&merged.cancelled_events[0], recovery_id);

        // Ordinary and timestamp-preserving requeue also retain cancelled
        // carryover, flattening it into the next normal dispatch without loss.
        assert!(queue.requeue(merged).is_none());
        queue.mark_complete(channel_id);
        queue.retry_after.remove(&channel_id);
        let requeued_merged = queue
            .flush_next()
            .expect("ordinary requeue must retain cancelled recovery event");
        assert_recovery(&requeued_merged.events[0], recovery_id);
        assert_eq!(requeued_merged.events[1].recovery_event_id, None);
        queue.requeue_preserve_timestamps(requeued_merged);
        queue.mark_complete(channel_id);
        let preserved_merged = queue
            .flush_next()
            .expect("timestamp-preserving requeue must retain recovery event");
        assert_recovery(&preserved_merged.events[0], recovery_id);
        assert_eq!(preserved_merged.events[1].recovery_event_id, None);

        // Cancelled-only fallback turns its recovery event back into events.
        let mut fallback_queue = EventQueue::new(DedupMode::Queue);
        let fallback_channel = Uuid::new_v4();
        fallback_queue.push(queued_recovery(fallback_channel, "fallback", recovery_id));
        let fallback = fallback_queue.flush_next().expect("fallback source batch");
        fallback_queue.requeue_as_cancelled(fallback, CancelReason::Interrupt);
        fallback_queue.mark_complete(fallback_channel);
        let fallback = fallback_queue
            .flush_next()
            .expect("fallback recovery batch");
        assert_recovery(&fallback.events[0], recovery_id);

        // Native steer release preserves provenance after the side-table route.
        let mut native_release_queue = EventQueue::new(DedupMode::Queue);
        let native_release_channel = Uuid::new_v4();
        let native_release_event =
            queued_recovery(native_release_channel, "native-release", recovery_id);
        let native_release_event_id = native_release_event.event.id.to_hex();
        native_release_queue.push(native_release_event);
        assert!(native_release_queue
            .mark_native_steer_pending(native_release_channel, &native_release_event_id));
        native_release_queue.release_native_steer(native_release_channel, &native_release_event_id);
        let native_release = native_release_queue
            .flush_next()
            .expect("released native-steer recovery batch");
        assert_recovery(&native_release.events[0], recovery_id);

        // Native steer expiry recovery preserves provenance after bulk release.
        let mut native_recovery_queue = EventQueue::new(DedupMode::Queue);
        let native_recovery_channel = Uuid::new_v4();
        let native_recovery_event =
            queued_recovery(native_recovery_channel, "native-recovery", recovery_id);
        let native_recovery_event_id = native_recovery_event.event.id.to_hex();
        native_recovery_queue.push(native_recovery_event);
        native_recovery_queue
            .in_flight_channels
            .insert(native_recovery_channel);
        native_recovery_queue.in_flight_deadlines.insert(
            native_recovery_channel,
            Instant::now() - Duration::from_secs(1),
        );
        native_recovery_queue
            .in_flight_batch_sizes
            .insert(native_recovery_channel, 1);
        assert!(native_recovery_queue
            .mark_native_steer_pending(native_recovery_channel, &native_recovery_event_id));
        assert!(native_recovery_queue.has_flushable_work());
        let native_recovery = native_recovery_queue
            .flush_next()
            .expect("expired native-steer recovery batch");
        assert_recovery(&native_recovery.events[0], recovery_id);

        // Splitting removes every recovery event once, retains normal ordering,
        // and preserves both regular and cancelled normal events.
        let cancelled_recovery_id = make_event("cancelled-recovery-origin").id;
        let batch_event = |content: &str, recovery_event_id| BatchEvent {
            event: make_event(content),
            prompt_tag: "test".into(),
            received_at: Instant::now(),
            recovery_event_id,
        };
        let (normal, recovery_ids) = FlushBatch {
            channel_id: Uuid::new_v4(),
            events: vec![
                batch_event("normal-1", None),
                batch_event("recovery-current", Some(recovery_id)),
                batch_event("normal-2", None),
            ],
            cancelled_events: vec![
                batch_event("cancelled-normal-1", None),
                batch_event("recovery-cancelled", Some(cancelled_recovery_id)),
                batch_event("cancelled-normal-2", None),
            ],
            cancel_reason: Some(CancelReason::Steer),
        }
        .split_recovery_events();
        assert_eq!(recovery_ids, vec![recovery_id, cancelled_recovery_id]);
        assert_eq!(
            normal
                .events
                .iter()
        ...
        assert_eq!(
            normal
                ...
                .map(|event| event.event.content.as_str())
                .collect::<Vec<_>>(),
            vec!["cancelled-normal-1", "cancelled-normal-2"]
        );
        assert_eq!(normal.cancel_reason, Some(CancelReason::Steer));
    }
