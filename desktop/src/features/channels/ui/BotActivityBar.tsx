...
import { RelayAgentModelPicker } from "@/features/agents/ui/RelayAgentModelPicker";
...
export function relayAgentModelPickerKey(
  agentPubkey: string,
  channelId: string,
  conversationRoot: string | null,
): string {
...
export function filterBotActivityTranscript<
  T extends Pick<TranscriptItem, "channelId" | "conversationRoot">,
>(
  ...
  return transcript.filter(
    (item) =>
      (channelId === null || item.channelId === channelId) &&
      // Root-less observer items are legacy channel-scoped activity and remain
      // visible as the documented fallback in a modern thread.
      (conversationRoot === null ||
        item.conversationRoot == null ||
        item.conversationRoot === conversationRoot),
  );
}
...
export function BotActivityComposerAction({
  ...
  const activityHeadlines = React.useMemo(() => {
    ...
    const scopedTranscript = filterBotActivityTranscript(
      transcript,
      channelId,
      conversationRoot,
    );
    ...
    return headlines;
  }, [channelId, conversationRoot, singleWorkingAgent, transcript]);
  ...
  return (
    <>
      ...
      {singleWorkingAgent && channelId ? (
        <RelayAgentModelPicker
          agentPubkey={singleWorkingAgent.pubkey}
          channelId={channelId}
          conversationRoot={conversationRoot}
          key={relayAgentModelPickerKey(
            singleWorkingAgent.pubkey,
            channelId,
            conversationRoot,
          )}
