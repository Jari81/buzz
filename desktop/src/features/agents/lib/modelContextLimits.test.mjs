...
test("provider prefixes are stripped before lookup", () => {
  assert.equal(resolveContextWindow("anthropic/claude-sonnet-4-20250514"), 200_000);
...
test("prefix match resolves dated model variants", () => {
  assert.equal(resolveContextWindow("qwen3-coder-480b-a35b-instruct-2025"), 262_144);
});
...
test("observed usage above the table value promotes to 1M", () => {
  ...
  assert.equal(resolveContextWindow("claude-sonnet-4-20250514", 250_000), 1_048_576);
});

test("observed usage at or below the table value keeps the table value", () => {
  assert.equal(resolveContextWindow("claude-sonnet-4-20250514", 150_000), 200_000);
  assert.equal(resolveContextWindow("claude-sonnet-4-20250514", 200_000), 200_000);
});
