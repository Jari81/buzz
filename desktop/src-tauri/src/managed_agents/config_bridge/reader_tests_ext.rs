...
// ── F2: consumed legacy effort must not double-emit in advanced (kalvin P2) ───
//
// When `build_thinking_field` consumes a valid record-tier legacy
// `BUZZ_AGENT_THINKING_EFFORT` (native key absent), the advanced loop must hide
// that same key — otherwise the panel shows the effort twice (normalized field
// AND a generic advanced env var). An invalid/unconsumed legacy value must stay
// visible in advanced so nothing is silently dropped.

/// Record has legacy `BUZZ_AGENT_THINKING_EFFORT=high` (valid) and no native
...
/// Record has legacy `BUZZ_AGENT_THINKING_EFFORT=bogus` (invalid for Goose) →
/// not consumed as effort, so it must remain VISIBLE in advanced (never dropped).
#[test]
...
// ── F4: find_effort_config_option must not fall through on unset preferred ────
//
// An advertised `thought_level` entry with `current_value: None` must NOT fall
// through to a set `effort` category — that would route the write to the wrong
// config_id. An unset preferred category returns None.

/// `thought_level` present but unset, `effort` present and set → effort must
/// NOT surface from the live cache (no fallthrough); the write route never
/// picks up the legacy `effort` config_id.
#[test]
...
/// fallback still works (a real transition adapter emits only `effort`).
#[test]
